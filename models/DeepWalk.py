...
class DeepWalk(nn.Module):
	def __init__(self):
		...
		self.node_embed = np.load("./embeddings/deepwalk/node_embedding_deepwalk.npy").item()
		self.logreg_coef = np.load("./embeddings/deepwalk/legreg_coef_dict_deepwalk.npy").item()

		self.entity_idx_to_dw_idx = {}
		self.relation_idx_to_dw_idx = {}
		with open('./datasets/FB15K237/entity2id.txt', 'r') as INPUT:
			for line in INPUT.readlines():
				pair = line.strip().split('\t')
				if len(pair) == 2 and pair[0] in node_dict:
					self.entity_idx_to_dw_idx[int(pair[1])] = node_dict[pair[0]]
		with open('./datasets/FB15K237/relation2id.txt', 'r') as INPUT:
			for line in INPUT.readlines():
				pair = line.strip().split('\t')
				if len(pair) == 2 and pair[0] in freq_dict:
					self.relation_idx_to_dw_idx[int(pair[1])] = freq_dict[pair[0]]

		self.dw_idx_to_entity_idx = dict((v,k) for k,v in self.entity_idx_to_dw_idx.iteritems())
		self.dw_idx_to_relation_idx = dict((v,k) for k,v in self.relation_idx_to_dw_idx.iteritems())"""
		self.logreg_models = []
		for edge_id in range(237):
			self.logreg_models.append(load('./embeddings/deepwalk/logreg/edge_' + str(edge_id) + '_logreg.joblib'))

		self._n_entities = 14541
		self._n_relations = 128
		self.node_embed = np.zeros((self._n_entities, self._n_relations))
		with open('./embeddings/deepwalk/fb15k237_deepwalk.emb', 'r') as INPUT:
			for line in INPUT.readlines():
	...
	def predict(self, h, t, r, pred_tail = True):
		#prob = []
		#h = self.entity_idx_to_dw_idx[h]
		#t = self.entity_idx_to_dw_idx[t]
		#r = self.relation_idx_to_dw_idx[r]
		...
		#return [self.dw_idx_to_entity_idx[samples[i]] for i in sorted(range(len(prob)), key = lambda k: prob[k], reverse = True)]
		if pred_tail:
	...
	def predict_relation(self, h, t, r):
		...
		#r = self.relation_idx_to_dw_idx[r]
		#relations, prob = [], []
		#for i in self.node_embed:
		#	if h in self.node_embed[i] and t in self.node_embed[i]:
		#		prob += [self.score(h, i, t)]
		#		relations += [i]
		#if len(relations) == 0:
		#	raise Exception
		#return [self.dw_idx_to_relation_idx[relations[i]] for i in sorted(range(len(prob)), key=lambda k: prob[k], reverse=True)]
